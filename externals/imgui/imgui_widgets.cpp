...
void ImGui::ProgressBarCustomColor(float fraction, const ImVec4& colorLeft, const ImVec4& colorBg, const ImVec2& size_arg, const char* overlay)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    // Render
    fraction = ImSaturate(fraction);
    RenderFrame(bb.Min, bb.Max, ColorConvertFloat4ToU32(colorBg), true, style.FrameRounding);
    bb.Expand(ImVec2(-style.FrameBorderSize, -style.FrameBorderSize));
    const ImVec2 fill_br = ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
    RenderRectFilledRangeH(window->DrawList, bb, ColorConvertFloat4ToU32(colorLeft), 0.0f, fraction, style.FrameRounding);
